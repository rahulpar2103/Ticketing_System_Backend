...
async def create_team(team: TeamCreate, current_user=Depends(get_current_user), db=Depends(get_db)):
    return team_service_admin.create_team(team, current_user, db)

@router.get('/get-all', response_model=list[TeamResponse])
async def get_all_teams(current_user=Depends(get_current_user), db=Depends(get_db), limit: int=10, offset: int=0):
...
async def delete_team(id: int, current_user=Depends(get_current_user), db=Depends(get_db)):
